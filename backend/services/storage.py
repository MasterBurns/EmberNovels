...
class StorageService:
    ...
    @classmethod
    def get_mindmap_file(cls, project_id: str) -> Path:
        return cls.get_projects_dir() / project_id / "mindmap.json"

    @classmethod
    def load_mindmap(cls, project_id: str) -> Dict[str, Any]:
        file_path = cls.get_mindmap_file(project_id)
        if not file_path.exists():
            return {"nodes": [], "edges": []}
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
    ...
    @classmethod
    def save_mindmap(cls, project_id: str, data: Dict[str, Any]) -> bool:
        file_path = cls.get_mindmap_file(project_id)
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=4, ensure_ascii=False)
