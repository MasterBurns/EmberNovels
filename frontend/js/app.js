...
async function updateMindmapEntitySelect() {
    ...
    if (type === 'custom') {
        ...
        try {
            if (type === 'character') {
                const res = await fetch(`/api/projects/${state.currentProject.id}/characters`);
                const chars = await res.json();
                chars.forEach(c => select.add(new Option(c.name, c.id)));
            } else if (type === 'lore') {
                const res = await fetch(`/api/projects/${state.currentProject.id}/lore`);
                const lore = await res.json();
                lore.forEach(l => select.add(new Option(l.title, l.id)));
